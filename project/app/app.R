...
server <- function(input, output) {
  h2o.init()
  model <- h2o.loadModel("../../project/4-model/gmb_grid")
  #model <- h2o.import_mojo("../4-model/GBM_1_AutoML_1_20221124_232404.zip")
  
  observeEvent(input$check,{
    #data <- reactive({
      #req(input$yearly_income)#
      #req(input$home_ownership)
      #req(input$bankruptcies)#
      #req(input$years_current_job)#
      #req(input$monthly_debt)
      #req(input$years_credit_history)
      #req(input$months_since_last_delinquent)#
      #req(input$open_accounts)
      #req(input$credit_problems)
      #req(input$credit_balance)
      #req(input$max_open_credit)#
      #req(input$amount_current_loan)
      #req(input$term)
      #req(input$credit_score)#
      #req(input$loan_purpose)
      data <- data.frame(                               
